import { atom, getDefaultEcosystem, injectAtomValue } from "@zedux/react";

import type { QueryAtomOptions, QueryFactoryTemplate } from "./_types";
...
export const queryAtom = <
  ...
  TError extends Record<string, unknown> = Record<string, unknown>,
...
>(
  ...
  queryTemplate: QueryFactoryTemplate<
    TQueryFnData,
    TAtomParams,
    TCombinedParams
  >,
  // Options are generic over TCombinedParams
  options?: QueryAtomOptions<TQueryFnData, TFinalData, TError, TCombinedParams>,
) => {
  ...
  const ttl = options?.ttl || ecosystem.get(queryConfigAtom).ttl;
  ...
  const factory = (...atomParams: TAtomParams) => {
    ...
    const throwOnError = !suspense ? false : _throwOnError;
    const maxRetries =
      typeof retry === "number"
        ? retry
        : retry === true
          ? configDefaults.maxRetries
          : 0; // Default retries

    // loadedState is TQueryDef<TQueryFnData, TCombinedParams>
    ...
    if (serializeQueryParams) {
      ...
      try {
        ...
        queryLog(
          true,
          `Query ${key}: Failed to stringify query params for cache key. Using baseKey only. Error:`,
          e,
        );
    ...
    const qapi = injectQuery<TQueryFnData, TFinalData, TError, TCombinedParams>(
      key,
      cacheKeyToUse,
      loadedState,
      {
